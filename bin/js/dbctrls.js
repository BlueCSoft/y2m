...
/*		
	 if(this.userBgColor&&this.field.$()!="")
	   this.element.style.background = "#"+this.field.$(); 
	 else 
	   this.element.style.background = "";    */
   }
  }
...
TDBEdit.prototype.doKeyDown = function(e){
 ...
 if(edit.oldOnKeyDown!=null)
  edit.oldOnKeyDown(edit,window.event);
   
 if(edit.OnKeyDown!=null)
  edit.OnKeyDown(edit,window.event);
 else{
  if(edit.Class!="TEXTAREA")
   edit.dataSource.keyDown(edit,window.event); 
 }
}
...
TDBEdit.prototype.doChange = function(e){
 var edit = this.owner;
...
TDBCheckBox.prototype.doKeyDown =function(e){
  ...
  if(vSrc.owner.oldOnKeyDown!=null)
   vSrc.owner.oldOnKeyDown(cbox,window.event);  
  if(vSrc.owner.OnKeyDown!=null)
   vSrc.owner.OnKeyDown(vSrc.owner,window.event)
  else
   vSrc.owner.dataSource.keyDown(vSrc.owner,window.event);   
}
...
TDBRadio.prototype.doKeyDown =function(e){
  ...
  if(vSrc.owner.oldOnKeyDown!=null)
   vSrc.owner.oldOnKeyDown(vSrc,window.event);
...
TDBComboBox.prototype.doCodeCallBack = function(dataset){
  ...
  if(this.displayIndex==1){	
   this.value = dataset.$("F1");	
   this.element.value = dataset.$("F2");
  }else{
   this.value = dataset.$("F2");	
   this.element.value = dataset.$("F1");
  }
...
TDBComboBox.prototype.readData = function(awOpen,qc){
  ...
  if(nResult>=2){ 
    ...
    $("#finddialogForm").css("width",this.width);
    $("#finddialogModalLabel").text("选择...");
    $("#finddialogBody").css("height",this.height);
...
TDBComboBox.prototype.doKeyDown =function(e){
  ...
  if(result){
   ...
   if(cbox.OnKeyDown!=null)
    ...
    cbox.dataSource.keyDown(cbox,window.event);    
  }
}
...
TDBDate.prototype.doKeyDown =function(e){
  ...
  if(cbox.OnKeyDown!=null)
   ...
   cbox.dataSource.keyDown(cbox,window.event);   
  if(window.event.keyCode!=13){
   //window.event.keyCode = 0;  有些浏览器此句会报错误
   return false;
  }
}
...
TDBSelect.prototype.doKeyDown = function(e){
 ...
 if(edit.OnKeyDown!=null)
  edit.OnKeyDown(edit,window.event);
 else edit.dataSource.keyDown(edit,window.event); 
}
/********************
*设置数据更改
*********************/
TDBSelect.prototype.updateData = function(){
 if(this.fieldNo.sField!=null){
  ...
  if(this.fieldName!=""){
   var n = this.element.selectedIndex;
