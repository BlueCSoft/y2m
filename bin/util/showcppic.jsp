...
%><%
  ...
  String cpcode = request.getParameter("cno").toString();
  try{
   query.queryBySql("select fileid from base_commodity "+
                    "where cp_code='%s'",cpcode,session);
   query.next();					
   String fileid = query.getString(1);
  
   query.queryBySql("select nfilename,ofilename,pid,convert(char(20),getdate(),120) stime "+
                    "from pub_files where id=%s",fileid,session);
   if(query.next()){
