...
function compareRecord(recordset,indexObj,sValue,dPos){
 ...
 recordset.absolutePosition = dPos;
 for(var i=0;i<indexObj.indexFieldCount;i++)
  oValue[i] = recordset(indexObj.shortNames[i]).value;
...
TIndexDef.prototype.initSort = function(){
  ...
  var k = 0;
  this.ts="";
  this.nRecNo = recordset.absolutePosition; 
  ...
  for(var i=1;i<=recordset.recordCount;i++){
   ...
   for(var j=0;j<this.indexFieldCount;j++)
    sValue[j] = recordset(this.shortNames[j]).value;
   this.addNode(recordset,sValue,i);
  }
  
  recordset.absolutePosition = this.nRecNo;
...
TIndexDef.prototype.addAdjust = function(sPos){
  ...
  for(var i=0;i<this.indexFieldCount;i++)
   sValue[i] = recordset(this.shortNames[i]).value;
...
/*构造函数*/
function TTable(xmlData,xmlMeta,xmlDelta,tableName,fixedTotal,props){

  this.className = "TTable";
  this.dataXml = xmlData;
  this.metaXml = xmlMeta;
  this.deltaXml = xmlDelta;
  this.pData  = xmlData.recordset;
  this.pMeta  = xmlMeta.recordset;
  this.pDelta = xmlDelta.recordset;
  ...
  this.fixedTotal = typeof(fixedTotal)!="undefined"&&fixedTotal;
...
function cloneRecord(dDataSet,sDataSet){
  var s = "";
  for(var i=0;i<sDataSet.fields.count;i++){
   s = sDataSet.fields(i).value;
   if(s!=null&&s!="")
    s = s.replace(/\n/g,"ん");
   dDataSet.fields(i).value = s;
  }
}
  <!--交换记录 从fromIndex到toIndex-->
function copyRecord(dataset,fromIndex,toIndex){
  dataset.absolutePosition = fromIndex;
  var buffer = new Array(dataset.fields.count);
  var i = 0;
  var s = "";
  for(i=0;i<dataset.fields.count;i++){  <!--复制到缓冲 -->
   s = dataset.fields(i).value;
   s = s.replace(/\n/g,"ん");
   buffer[i] = s;
  }

  dataset.absolutePosition = toIndex;

  for(i=0;i<dataset.fields.count;i++)  <!--缓冲到目标 -->
   dataset.fields(i).value = buffer[i];
   
  buffer = null; 
}
  <!--定位记录-->
function locateRecord(dataset,rLocation,rType,fromIndex){
  if(fromIndex<=dataset.recordCount){
   dataset.absolutePosition = fromIndex;  <!--移动到第一条记录 -->
   while(!dataset.EOF){
    if(parseInt(dataset("RL"))==parseInt(rLocation)&&
       parseInt(dataset("RT"))==parseInt(rType))
     return dataset.absolutePosition;
...
TTable.prototype.initTable = function(){
  this.recordValue = new Array;
  this.fields = new Array(this.pMeta.recordCount);  //字段数组
  ...
  //设置字段数组
  var i = 0,j=0,mdSet = this.pMeta,Field;
  ...
  while(!mdSet.EOF){
   ...
   Field.index = i; //字段索引
   Field.dataset = this;
   Field.sField = this.pData(mdSet("O").value);
   Field.fieldName = mdSet("A").value.toUpperCase(); //字段名称
   Field.shortName = mdSet("O").value.toUpperCase();   
   Field.keyField = mdSet("K").value=="1"; //主键字段
   Field.dataType = parseInt(mdSet("B").value,10);  //数据类型
   Field.size = parseInt(mdSet("W").value,10);
   Field.isNull = (mdSet("D").value=="1"||mdSet("D").value=="false");
   Field.precision = parseInt(mdSet("P").value,10);
   if(Field.dataType==2||Field.dataType==3){
     Field.precision = 2;
	 Field.displayFormat = "0.00";
   }
	
   Field.readOnly = mdSet("R").value=="1";
   Field.displayLabel = mdSet("L").value;
   Field.displayWidth = parseInt(mdSet("W").value);
   if(mdSet("M").value!="")
     Field.displayFormat = mdSet("M").value;
   var cstr = mdSet("C").value;
   if(cstr!=""&&cstr!=null)
     cstr = cstr.replace(Field.fieldName,"v");
   Field.constrant = cstr;
   Field.errorMsg = mdSet("E").value;
   Field.format = mdSet("F").value;
   Field.formatErr = mdSet("H").value;
  ...
  this.pData.absolutePosition = 1;
  this.pDelta.absolutePosition = 1;
  ...
  this.__SeqId = 0;
  cloneRecord(this.pDelta,this.pData);  	
...
TTable.prototype.reInitTable = function(xmlObj){
  ...
  this.indexDefs = null;
  this.recordValue = null;
  //alert(xmlObj.childNodes[0].childNodes[0].xml);
  var Obj = xmlObj.childNodes[0];
  
  //alert(Obj.childNodes[2].childNodes[0].xml);
  this.deltaXml.loadXML(Obj.childNodes[0].childNodes[0].xml);
  this.metaXml.loadXML(Obj.childNodes[1].childNodes[0].xml);
  this.dataXml.loadXML(Obj.childNodes[2].childNodes[0].xml);
  
  this.pData = this.dataXml.recordset;
  this.pMeta  = this.metaXml.recordset;
  this.pDelta = this.deltaXml.recordset;  
...
TTable.prototype.getState = function(){
  if(this.pData("RS").value=="")
   return 0;
  else return parseInt(this.pData("RS").value); 
}

TTable.prototype.getRecordType = function(){
  if(this.pData("RT").value==""||this.pData("RT").value=="0")
   return 0;
  var n = parseInt(this.pData("RT").value); 
  if(n<=2) n=1;
  return n;
}
...
TTable.prototype.innerInsert = function(sRows,delRows,bSelected){
 ...
 this.dataXml.loadXML(sRows.xml);
 this.pData = this.dataXml.recordset;

 for(var i=0;i<this.fields.length;i++)
   this.fields[i].sField = this.pData(this.fields[i].shortName); 
  ...
  if(mresult&&dSet.post()) //首先Post目前记录的数据
   if(dSet.BeforeInsert==null||dSet.BeforeInsert(dSet)){  //首先检查并调用增加前事件
    ...
    if(this.SeqFieldName!=""&&this.SeqName!=""){
	  this.__SeqId = _$GetSeq(this.SeqName);
    ...
    if(mresult){ 
     ...
     this.isInNew = true;
     if(parseInt(data("RT"))>0){ //增加新记录
      data.addNew();
 	 }

     data("RT").value = "3";    <!--类型变成 3 - 新记录 -->
     data("RS").value = "2";    <!--状态变成 2 - 插入状态 -->
     data("RR").value = "0";

     if(dSet.R_Location==0) dSet.R_Location = data.recordCount;  //设置记录标识
	 else dSet.R_Location++;
     data("RL").value = ""+dSet.R_Location;  <!--内部记录定位号-->
     ...
     if(this.SeqFieldName!=""&&this.SeqName!="")
	  dSet.innersetFieldValue(this.SeqFieldName,this.__SeqId);
     ...
     for(var i=0;i<data.fields.count;i++)   //把空值设置为空白
      if(data.fields(i).value==null)
       data.fields(i).value = "";

     //清空旧值
	 this.isInNew = false;
     this.innermoveRecord(data.recordCount,"new");
  ...
  if(mresult&&dSet.post()) //首先Post目前记录的数据
   if(dSet.BeforeInsert==null||dSet.BeforeInsert(dSet)){  //首先检查并调用增加前事件
    ...
    if(mresult){ 
     ...
     this.isInNew = true;
     if(parseInt(data("RT"))>0){ //增加新记录
	  var orecNo = data.absolutePosition; 
      data.addNew();
      for(var i=data.recordCount;i>orecNo;i--)
 	   copyRecord(data,i-1,i);
	  data.absolutePosition = orecNo;  
	 ...
	 //this.innermoveRecord(data.absolutePosition,"new");
     this.innermoveRecord(data.absolutePosition,"move");	
...
TTable.prototype.Delete = function(){
  ...
  if(parseInt(data("RT"))>0)
   if(dSet.BeforeDelete==null||dSet.BeforeDelete(dSet)){
    ...
    switch(parseInt(data("RT").value)){
        case 1:<!--未被修改的记录 -->
               var dRecNo = delta.addNew();
               data("RT").value = "4";   <!--被删除的记录-->
               cloneRecord(delta,data); <!--复制data到delta-->
               break;
        case 2:<!--被修改了的记录 -->
               var n = locateRecord(delta,data("RL").value,"1",2);<!--查找类型为2的记录-->
               if(n>0){     <!--n>0表示本次修改已经被记录在delta中，要先处理delta -->
                delta.absolutePosition = n;  <!--定位delta-->
                delta("RT").value = "4";  <!--设置成被删除标记 -->
                delta.absolutePosition = n+1;
                delta.Delete();         <!--删除被修改的-->
               } else {
                delta.absolutePosition = 1;
                delta("RT").value = "4";
                var dRecNo = delta.addNew();
                copyRecord(delta,1,delta.absolutePosition);  <!--复制原记录-->
               }
               break;
        case 3:<!--新增加的记录-->
               var n = locateRecord(delta,data("RL").value,"3",2);<!--查找类型为2的记录-->
    ...
    var x = data.absolutePosition;
    this.indexDefs.delAdjust(x); 		

    if(data.recordCount==1){
     for(var i=4;i<data.fields.count;i++)
      data.fields(i).value = "";
     ...
     data.Delete(); <!--删除当前记录-->
     if(x>data.recordCount) {
       x = data.recordCount;
       data.absolutePosition = x;
     }
	}
...
TTable.prototype.editRow = function(){
  ...
  if(data("RT").value=="0")  //空闲记录 
   result = dSet.insert();
  else{
   rsState = data("RS").value=="0"; 	   

   result = !rsState||dSet.BeforeEdit==null||dSet.BeforeEdit(dSet);
	
   if(result)
    if(rsState){
     data("RS").value = "1";   <!-- 设置为编辑状态 -->	
...
TTable.prototype.cancelRow = function(){
  ...
  var delta = dSet.pDelta;
  var result = false;
  if(parseInt(data("RT"))>0&&parseInt(data("RS"))>0)
   if(dSet.BeforeCancel==null||dSet.BeforeCancel(dSet)){
    switch(parseInt(data("RS").value)){
        case 1: <!--修改状态-->
                delta.absolutePosition = 1;
                cloneRecord(data,delta); <!--复制delta到data-->
                data("RS").value = "0";
				dSet.innermoveRecord(data.absolutePosition,"post");	
                break;
        case 2: <!--插入状态-->
                this.deleteRow(); <!--直接删除-->
    }
...
TTable.prototype.emptyDataset=function(){
  ...
  while(delta.recordCount>1){
   delta.absolutePosition = delta.recordCount;
   delta.Delete();
  }
 //清空索引
  this.indexDefs.clearNode();
...
TTable.prototype.resetData=function(){
  ...
  var metadata = dSet.pMeta;
  var delta = dSet.pDelta;
  var n = data.absolutePosition; <!--原来的位置-->
  <!--处理数据-->
  data.moveFirst();
   while(!data.EOF){
    if(parseInt(data("RT"))>1) data("RT").value = "1";     <!--设置旧记录标记 -->
    data.moveNext();
   }
  data.absolutePosition = n;
  <!--处理变化的数据-->
  while(delta.recordCount>1){
   delta.absolutePosition = delta.recordCount;
   delta.Delete();
...
TTable.prototype.Validate = function(){
  ...
  for(var i=4;result&&i<data.fields.count;i++){
   var oldValue = delta.fields(i).value;
   sfield = data.fields(i);
   //if(oldValue!=sfield.value)
...
TTable.prototype.post =  function(){
 ...
 // if(!this.__saveDataStatus)
  if(data("RS").value=="0"||data("RS").value=="null")
    return true;
  if(dSet.BeforePost==null||dSet.BeforePost(dSet)){
   ...
   if(this.__postmark&&dSet.Validate()){
    <!--此处加入确认操作代码-->
	result = true;
    switch(parseInt(data("RT").value)){ <!-- 3 新记录 -->
        case 3:
		       if(data("RS")=="2"){   <!-- 2 插入状态 把数据存到Delta中-->
			   
			    result = this.indexDefs.addAdjust(data.absolutePosition)==0; 		       
        ...
        case 2:<!--修改的记录(重复修改)-->
	           result = this.indexDefs.editAdjust(data.absolutePosition)==0; 		       
               if(!result) break;
			   
               var n = locateRecord(delta,data("RL").value,data("RT").value,2);
               if(n>0){
                data("RS").value = "0";
                delta.absolutePosition = n;
                cloneRecord(delta,data);
               }
               break;
        case 1:<!--修改的记录(第一次修改)-->
		       //调整索引
...
TTable.prototype.isEmpty = function(){
  var dSet = this;
  return (dSet.pData.recordCount==0||dSet.pData("RT").value == "0");
}
/*检查记录时候在最后*/
TTable.prototype.isLast = function(){
  var dSet = this;
  return dSet.pData.recordCount==dSet.pData.absolutePosition;
}
...
TTable.prototype.innermoveRecord = function(toRow,att){
  ...
  var data = dSet.pData,metadata = dSet.pMeta,delta = dSet.pDelta;
  var result = true;
  var bMark = data.absolutePosition != toRow;
  
  data.absolutePosition = toRow;

  if(bMark){
   for(var i=0;i<dSet.fields.length;i++)
    dSet.fields[i].oValue = data.fields(i+4).value;	
  }
  
  if(data("RS").value=="0"){
    delta.absolutePosition = 1;      <!--复制记录到缓冲中-->
    cloneRecord(delta,data);
   }
...
TTable.prototype.moveRecord = function(toRow){
  ...
  if(toRow<1) toRow = 1;
  if(toRow>data.recordCount) toRow = data.recordCount;

  if(data.absolutePosition != toRow){ <!--已经相等了不需要移动-->
 
   result = this.postAll();

   if(result){
    data.absolutePosition = toRow;  <!--移动记录到指定的位置-->
    delta.absolutePosition = 1;      <!--复制记录到缓冲中-->
    cloneRecord(delta,data);
...
TTable.prototype.recNo = function(){
  return this.isEmpty()? 0:this.pData.absolutePosition;	
}

TTable.prototype.isBof = function(){
 return this.isEmpty()||this.pData.absolutePosition==1;
}

TTable.prototype.isEof = function(){
 return this.isEmpty()||this.pData.absolutePosition==this.pData.recordCount;
}

 <!--第一条记录-->
TTable.prototype.first = function(){
  var result = false;
  if(this.pData.absolutePosition>1&&this.post())
...
TTable.prototype.next = function(){
  var dSet = this;
  var result = true;
  if(dSet.pData.absolutePosition<dSet.pData.recordCount&&dSet.post()){
   result = dSet.innermoveRecord(dSet.pData.absolutePosition+1,"move");
  }
  return result;
}
...
TTable.prototype.prior = function(){
  var dSet = this;
  var result = false;
  if(dSet.pData.absolutePosition>1&&dSet.post())
   result = dSet.innermoveRecord(dSet.pData.absolutePosition-1,"move");
  return result;
}
...
TTable.prototype.last = function(){
  ...
  var result = false;
  if(dSet.pData.absolutePosition<dSet.pData.recordCount&&dSet.post())
   result = dSet.innermoveRecord(dSet.pData.recordCount,"move");
  return result;
}
...
TTable.prototype.getRecordCount = function(){
  if(this.isEmpty())
   return 0;
  else return this.pData.recordCount;
}
...
TTable.prototype.locate = function(fieldName,value){
  ...
  for(var i=1;!result&&i<=dSet.pData.recordCount;i++){
   dSet.pData.absolutePosition = i;
   if(isDate){
    var s = dSet.pData(sName).value;
    s = s.replace(/\-/g,"/");
	s = s.replace(/\./g,"/");
    var d1 = new Date(s);
    result = d1.getDate()==d2.getDate();
   }else
    result = dSet.pData(sName).value==value;
   n = i;
  }
...
TTable.prototype.locates = function(fieldNames,values){
  ...
  for(var i=0;i<fieldNames.length;i++)
   fieldNames[i] = this.fieldByName[fieldNames[i].toUpperCase()].shortName;
  ...
  for(var i=1;!result&&i<=dSet.pData.recordCount;i++){
   ...
   for(var j=0;j<fieldNames.length;j++)
    tempFValue += dSet.pData(fieldNames[j]).value;
...
TTable.prototype.locates2 = function(fieldNames,values){
  ...
  for(var i=1;!result&&i<=dSet.pData.recordCount&&i!=nRecno;i++){
   dSet.pData.absolutePosition = i;
   tempFValue = "";
   for(var j=0;j<fieldNames.length;j++)
    tempFValue += dSet.pData(fieldNames[j]).value;
...
TTable.prototype.changeRecord = function(nFrom,nTo){
  ...
  if(this.isEmpty()||nFrom>data.recordCount||nTo>data.recordCount||nFrom<1||nTo<1)
   return;

  var tvf = new Array(data.fields.count);
  var tvt = new Array(data.fields.count);

  data.absolutePosition = nFrom;
  for(var i=0;i<data.fields.count;i++)
   tvf[i] = data.fields(i).value;

  data.absolutePosition = nTo;
  for(var i=0;i<data.fields.count;i++){
   tvt[i] = data.fields(i).value;
   data.fields(i).value = tvf[i];
  }
...
TTable.prototype.calSumValue = function(att){
  ...
  if(!dSet.fixedTotal){
   if(dSet.calTotal||(typeof(att)!="undefined"&&att)){
    var data = dSet.pData;
    var recno = data.absolutePosition;
    var i = 0;
     ...
     while(!data.EOF){
      for(i=0;i<dSet.fields.length;i++)  <!--合计设置为 -->
       if(dSet.fields[i].dataType >= 2&&dSet.fields[i].dataType<=5){ <!--数字类型 -->
        var value = data.fields(i+4).value;
    ...
    switch(att){
     case 0: <!--增加-->
          for(var i=0;i<dSet.fields.length;i++)
           if(dSet.fields[i].dataType >= 2&&dSet.fields[i].dataType<=5){
			c = nvl(dSet.fields[i].sumValue,0)+nvl(parseFloat(data.fields(i+4).value,10),0);    
     ...
     case 1: <!--修改-->
	      if(data.recordCount==1){
           for(var i=0;i<dSet.fields.length;i++)		  
            if(dSet.fields[i].dataType >= 2&&dSet.fields[i].dataType<=5){
		 	 b = nvl(parseFloat(data.fields(i+4).value,10), 0);
...
TTable.prototype.getMaxValue = function(fieldName){
  dSet = this;
  var data = dSet.pData;
  var n = data.absolutePosition;
  var nMax = "",mValue = ""
  var nnMax = 0,nmValue = 0;
  fieldName = fieldName.toUpperCase();
  var field = this.fieldByName[fieldName];
  fieldName = field.sField.name;
  data.moveFirst();
  
  if(!field.isNumberType()){
   while(!data.EOF){
    mValue = data(fieldName).value;
    nMax = (nMax<mValue)?mValue:nMax;
    data.moveNext();
   }
  }else{
   while(!data.EOF){
    nmValue = field.getFieldValue();
...
/*   if(dataType==1||dataType==12)
   ...
   if(dSet.editRow()){  //设置为编辑状态
	...
	if(result){
     ...
     //处理合计
     if(bDcce&&!this.lockField&&field.OnChange!=null&&!field.OnChanging){
...
TTable.prototype.setFieldValueAll = function(fieldName,value){
  ...
  dSet.post();
  for(var i=1;i<=dSet.pData.recordCount;i++){
   this.moveRecord(i);
...
TTable.prototype.getFieldValueByRow = function(fieldName,offRow,defaultValue){
  var n = this.pData.absolutePosition+offRow;
  var value = defaultValue;
  if(offRow!=0&&n>=1&&n<=this.pData.recordCount){
   this.pData.absolutePosition = n;	   
   fieldName = fieldName.toUpperCase();
   value = this.fieldByName[fieldName].getFieldValue(); 
   this.pData.absolutePosition = n-offRow;	
  }
  return value;
}
...
TTable.prototype.setFieldProp = function(fieldName,propName,value){
  ...
  for(var i=1;i<=meta.recordCount;i++){
   meta.absolutePosition = i;  
   if(fieldName==meta("A").value){
	meta(propName).value = value+""; 	
	break;
   }
  }
}
...
TTable.prototype.getChangeCount = function(){
  return this.pDelta.recordCount-1;	
}

TTable.prototype.dataIsChange = function(){
  var result = this.pDelta.recordCount-1>0||this.getState()>$datasetState.dsBrowse;
  var cdSet;
...
TTable.prototype.refrashData = function(xmlData,bAppend,bSelected){
 ...
 var datasetnode = root;
  
 var rownodes = datasetnode.childNodes;
 var recordCount = rownodes.length;
 var fieldCount = rownodes[0].attributes.length;
 var isEmpty = (recordCount==0)||(rownodes[0].getAttribute("RL")=="0");
 
 //alert(isEmpty);
 if(!isEmpty)
  this.innerInsert(root,0,bSelected);
 else{ 
...
//数据保存的方法
TTable.prototype.applyUpdate = function(noHint){
  var dSet = this;
  ...
  if(xmlDoc!=""){
   ...
   if(oXmlHttp.status==200){
    ...
    if(root.nodeName=="errors"){ //出现异常
     ...
     //__xmlHttp.pageFinishi2(root.childNodes[0].childNodes[0].nodeValue);
	 
     hInt = "数据保存成功.";  
     dSet.doResetAll();  //复位delta
	 dSet.doAfterUpdateAll();
	 result = 0;
	 //window.status = "start:"+t0+","+this.lastError+",end:"+getServerDateTime().substring(11);
	 //alert(hInt+"\n"+window.status);
	}
   }else{
...
TTable.prototype.applyUpdate2 = function(ofName,nfName,noHint){
 ...
 this.Reset();
 var nRecNo = DataSet.absolutePosition;
 for(var i=1;i<=DataSet.recordCount;i++){
  DataSet.absolutePosition = i;
  var ov=parseInt(DataSet(ofName).value,10);
  var nv=parseInt(DataSet(nfName).value,10);
  if(nv==-1) nv=1;
  if(ov!=nv){ //和原来的不一样,变化数据记录集中增加一条记录
   DeltaSet.addNew();
   cloneRecord(DeltaSet,DataSet);  //DataSet数据复制到DeltaSet
   if(ov==0)  //原来没有选中,现在选中了应该增加
     DeltaSet("RT").value = "3"; //增加标记
   else
     DeltaSet("RT").value = "4"; //删除标记
   isSave = true;
   DataSet(ofName).value = DataSet(nfName).value;
  }
 }
 ...
 for(var i=1;i<=DataSet.recordCount;i++){
  ...
  if(ov!=nv){ //和原来的不一样,变化数据记录集中增加一条记录
   ...
   if(ov==-1)  //原来没有选中,现在选中了应该增加
     ...
     DeltaSet(nfName).value = ov;
     DeltaSet("RT").value = "1"; //修改
     DeltaSet.addNew();
     cloneRecord(DeltaSet,DataSet);  //DataSet数据复制到DeltaSet
     DeltaSet("RT").value = "2"; //修改
   }
   isSave = true;
   DataSet(ofName).value = DataSet(nfName).value;
  }
...
TTable.prototype.saveRecord = function(){
 if(!this.isEmpty()){	
  ...
  var n = this.pData.fields.count;	
  for(var i=0;i<n;i++)
   this.recordValue[i] = this.pData.fields(i).value;
 }
}
...
TTable.prototype.totalRecordNum = function(fieldNames,values){
 ...
 var nRecNo = this.pData.absolutePosition;
 for(var i=1;i<=this.pData.recordCount;i++){
  this.pData.absolutePosition = i;	 
  var isOk = true; 
 ...
 this.pData.absolutePosition = nRecNo;
 return n;	
}
...
TTable.prototype.doSort = function(fieldName,sortAtt){
  ...
  var buf = new Array(dataset.recordCount);
  var buf2 = new Array(dataset.recordCount);

  for(i=dataset.recordCount;i>1;i--){
   dataset.absolutePosition = i;
   str1 = dataset(fieldName).value;
   for(j=i-1;j>=1;j--){
    dataset.absolutePosition = j;
    if(conpareFun(str1,dataset(fieldName).value)){
     str1=dataset(fieldName).value;
     k = j;
    }
   }
   if(k>0){
    dataset.absolutePosition = k;
    for(j=0;j<dataset.fields.count;j++)
     buf[j]=dataset.fields(j).value;
    dataset.absolutePosition = i;
    for(j=0;j<dataset.fields.count;j++){
     buf2[j]=dataset.fields(j).value;
     dataset.fields(j).value = buf[j];
    }
    dataset.absolutePosition = k;
    for(j=0;j<dataset.fields.count;j++)
     dataset.fields(j).value = buf2[j];
    k = 0;
   }
  }
