...
//		    +  '" href="javascript:docallfun(\'' + node.url + '\')"';
		...
		if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc))
  		 ...
  		 str += ' onclick="javascript:docallfun(\''+node.name+'\',\'' + node.url + '\')" style="cursor:hand;"';
...
dTree.prototype.indent = function(node, nodeId) {
	...
	if (this.root.id != node.pid) {
		...
		for (var n=0; n<this.aIndent.length; n++)
			...
			str += '<img src="' + ( (this.aIndent[n] == 1 && this.config.useLines) ? this.icon.line : this.icon.empty ) + '" alt="" align="absmiddle"/>';

		(node._ls) ? this.aIndent.push(0) : this.aIndent.push(1);

		if (node._hc) {

			str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');"><img id="j' + this.obj + nodeId + '" align="absmiddle" src="';
		...
		} else str += '<img src="' + ( (this.config.useLines) ? ((node._ls) ? this.icon.joinBottom : this.icon.join ) : this.icon.empty) + '" alt="" align="absmiddle"/>';
